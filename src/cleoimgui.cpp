...
float CLEOImGui::m_fImGuiVersion = 0.0f;
float CLEOImGui::m_fFontSize = 0.0f;
...
void CLEOImGui::RegisterOpcodes()
{
	if (CLEO_GetVersion() >= CLEO_VERSION) 
	{
		...
		Hook::windowCallback = std::bind(&DrawImGui);
	}
	else
	{
...
void CLEOImGui::DrawImGui()
{
	...
	// update stuff
	Hook::m_bShowMouse = ScriptExData::show_cursor;
	m_fFontSize = ImGui::GetFontSize();
}


CLEOImGui::CLEOImGui()
{	
	m_fImGuiVersion = std::stof(ImGui::GetVersion());
